...
def url_to_filename(url):
    ...
    try:
        ...
        traceback.print_exc()
        sys.stderr.flush()
        sys.stderr.write("url = '%s', distro = '%s', ver=%s\n" % (url, distro, ver))
        raise
    #print "distro = '%s', ver = '%s', tag = '%s', url = '%s'" % (distro, ver, tag, url[57:])
...
def get_repo_urls(osc_dir):
    ...
    old_cwd = os.getcwd()
    os.chdir(osc_dir)
    repourls = subprocess.Popen(["osc", "repourls"], stdout=subprocess.PIPE).communicate()[0]
