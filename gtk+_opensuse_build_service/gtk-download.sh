...
rm -f "$REPOTMP"
wget "$REPOURL" -nv -O - | grep -Eo \"m.*rpm\" | grep -Eo ming.*rpm | grep -vE '(debug|devel|gcc|filesystem|cpp|gmp|mpfr|mingw32-runtime|termcap|headers|mingw32-jpeg|iconv|mingw32-tiff|gettext-tools|jasper| mingw32-sqlite)' > "$REPOTMP"
