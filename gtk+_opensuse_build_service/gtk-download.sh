...
rm -f "$REPOTMP"
wget "$REPOURL" -nv -O - | grep -Eo \"m.*rpm\" | grep -Eo ming.*rpm | grep -vE '(debug|devel|gcc|filesystem|cpp|gmp|mpfr|pixmap|mingw32-runtime|termcap|pixman|headers|mingw32-jpeg|iconv|mingw32-tiff|mingw32-jasper|gettext-tools)' > "$REPOTMP"
